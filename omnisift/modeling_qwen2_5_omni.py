...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    ...
    def get_rope_index(
        ...
        """
        spatial_merge_size = self.spatial_merge_size # 2
        image_token_id = self.config.image_token_id
        video_token_id = self.config.video_token_id
        audio_token_id = self.config.audio_token_id
        vision_start_token_id = self.config.vision_start_token_id
        audio_start_token_id = self.config.audio_start_token_id
        position_id_per_seconds = self.config.position_id_per_seconds # 25
        seconds_per_chunk = self.config.seconds_per_chunk # 2
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                vision_tokens = input_ids[vision_start_indices + 1] # The token after video_start_token is audio_start_token.
                ...
                for _ in range(multimodal_nums):
                    ...
                    if image_token_id in input_tokens and remain_images > 0:
                        ...
                        ed_image = len(input_tokens) + 1 # len(input_ids)
                    if video_token_id in input_tokens and remain_videos > 0:
                        ed_video = input_tokens.index(video_token_id, st) # Index of the first video_token occurrence.
                    else:
                        ed_video = len(input_tokens) + 1
                    if audio_token_id in input_tokens and remain_audios > 0: 
                        ed_audio = input_tokens.index(audio_token_id, st) # 1485
                    else:
                    ...
                    if min_ed == ed_audio:
                        ...
                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                        bos_len = 1
                        llm_pos_ids_list.append(torch.arange(bos_len).view(1, -1).expand(3, -1) + st_idx) # <video_eos>
                        llm_pos_ids_list.append(torch.arange(bos_len).view(1, -1).expand(3, -1) + st_idx) # <audio eos>

                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                        audio_len = ((audio_seqlens[audio_idx] - 1) // 2 + 1 - 2) // 2 + 1 # 795
...
class TransformerAudioSelector(nn.Module):
    def __init__(self, d_model=3584, nhead=8, num_layers=1, hidden_dim=512):
        ...
        # Audio as Query, Video as Key/Value
    ...
    def forward(self, v_embeds, a_embeds):
        """
        v_embeds: [Lv, D] -> Context (Video)
        a_embeds: [La, D] -> Targets (Audio)
        """
        v = self.v_norm(self.v_proj(v_embeds)).unsqueeze(0) # [1, Lv, H]
        a = self.a_norm(self.a_proj(a_embeds)).unsqueeze(0) # [1, La, H]
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        # add a compressor
        self.compressor = TransformerAudioSelector()
    ...
    def forward(
        ...
        # OmniZip Inference
        # import ipdb; ipdb.set_trace()
        if pixel_values_videos is not None and audio_features is not None:
            from omnisift.compression_units import similarity_pruning
            if self.compression_config is not None:
                print("Using given compression config")
                compression_config = self.compression_config
            else:
                print("Using default compression config")
                compression_config = {
                    "rho_audio": 0.3,
                    "rho_video": 0.7,
                }
            inputs_embeds, global_mask, v_count, a_count = similarity_pruning(
                self.compressor,
                inputs_embeds,
                input_ids,
                ...
                position_ids,
                merging_ratio_a=compression_config["rho_audio"],
                merging_ratio_v=compression_config["rho_video"],
            )
            self.global_mask = global_mask

            inputs_embeds = torch.stack([
                inputs_embeds[b, global_mask[b]]
                for b in range(inputs_embeds.size(0))
            ], dim=0)

            if attention_mask is not None:
                attention_mask = torch.stack([
                    attention_mask[b, global_mask[b]]
                    for b in range(attention_mask.size(0))
                ], dim=0)
            position_ids = position_ids.to(inputs_embeds.device)

            position_ids = torch.stack([
                position_ids[:, b, global_mask[b]]
                for b in range(global_mask.size(0))
            ], dim=1)
            if labels is not None:
                labels = torch.stack([
                    labels[b, global_mask[b]]
                    for b in range(labels.size(0))
                ], dim=0)
            # import ipdb; ipdb.set_trace()
            # [Instrumentation] Log token reduction ratio
            if is_prefill and 'global_mask' in locals():
                # import ipdb; ipdb.set_trace()
                # original_len = global_mask.numel()
                # kept_len = global_mask.sum().item()
                self.original_a_tokens_num = len(audio_features)
                self.original_v_tokens_num = len(video_embeds)
                original_len = self.original_a_tokens_num + self.original_v_tokens_num
                self.compressed_a_tokens_num = a_count
                self.compressed_v_tokens_num = v_count
                kept_len = self.compressed_a_tokens_num + self.compressed_v_tokens_num
                self.keep_ratio = (kept_len/original_len)*100
                print(f"[Metrics] Token kept: {original_len} -> {kept_len} ({(kept_len/original_len)*100:.2f}%)")
        # import ipdb; ipdb.set_trace()
        outputs = self.model(
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        if should_time_prefill:
            ...
            # Store the latency in the model instance so we can read it later
            self.last_prefill_latency_ms = prefill_start_event.elapsed_time(prefill_end_event) / 1000
            print(f"[Metrics] Prefill Latency (Internal): {self.last_prefill_latency_ms:.2f} s")
    ...
    def build_video_prune_mask(
        ...
        """
        Prune video tokens only and return a one-dimensional keep_mask (N,).
        Overview:
        1) Split each video segment into frames: four frames if divisible by 4,
        otherwise two frames. If two adjacent segments both contain two frames,
        merge them across the intervening audio into one four-frame group.
        2) Align neighboring frames within each group by identical offsets and
        compute cosine similarities (A1-A2, A2-A3, A3-A4). The average
        similarity to neighboring frames is used as the frame-level non-key
        score. The frame with the lowest score is treated as the key frame;
        ties may produce more than one key frame.
        3) For non-key frames, use the adjacent frame toward the nearest key
        frame (left or right) as reference and use aligned per-position
        similarity as the redundancy score.
        4) Key frames are not pruned by default. If keyframe_audio_mode != 'none',
        use the following audio segment for cross-modal scoring:
        - Average the audio segment over batch and tokens, then compute
          per-position cosine similarity between each key frame and that mean.
        - keyframe_audio_mode='prune_high': higher similarity is more likely
          to be pruned, assuming strongly audio-video-correlated positions can
          be represented by audio or other frames.
        - keyframe_audio_mode='prune_low': lower similarity is more likely to
          be pruned, assuming weakly audio-related positions are less useful.
        5) Aggregate scores over all video positions and remove
        round(k * #video) positions from high to low. Non-video positions are kept.

        Args:
            k: Overall pruning ratio for video tokens (0 to 1).
            normalize: Whether to apply L2 normalization before cosine similarity
                computation. Defaults to True.
            keyframe_audio_mode: 'none' | 'prune_high' | 'prune_low'
                - Try both 'prune_high' and 'prune_low' and compare results.

        Returns:
            keep_mask: (N,) bool tensor, where True means keep and False means prune.
        """
        ...
        # Preprocess and optionally normalize.
        if normalize:
            eps = 1e-6
        ...
        # Mark whether video/audio tokens appear along the sequence dimension.
        is_video = (input_ids == video_token_id).any(dim=0)  # (N,)
        is_audio = (input_ids == audio_token_id).any(dim=0)  # (N,)

        # Find contiguous video segments.
        segments = []
        in_seg, st = False, 0
        for i in range(N):
            if is_video[i]:
        ...
        # Split by frames: divisible by 4 -> 4 frames; otherwise -> 2 frames;
        ...
        def split_frames(seg):
            ...
            # Very short: one frame.
            return [(s, e)]

        # Merge adjacent two-frame video segments across audio into a four-frame group.
        groups = []  # Each group contains frame spans [(s,e), ...] of length 4, 2, or 1.
        # Record the audio segment after each video segment for cross-modal key-frame scoring.
        audio_after_segment = {}  # seg_index -> (a_s, a_e) or None

        # Enumerate all audio segments first to query the following audio segment.
        audio_segments = []
        in_a, sa = False, 0
        for i in range(N):
            if is_audio[i]:
        ...
        # Find the nearest audio segment after each video segment.
        ...
        while i < len(segments):
            ...
            if len(fr) == 2 and (i + 1) < len(segments):
                ...
                if len(fr_next) == 2:
                    # Merge into a four-frame group.
                    groups.append({
                        "frames": fr + fr_next,
                        # Use the audio after the second segment as the group's following audio.
        ...
        # Helper: aligned per-position similarity between two frames.
        def pairwise_sims(frame_a, frame_b):
            ...
            v1 = emb[:, idx_a, :]   # (B, L, D)
            v2 = emb[:, idx_b, :]   # (B, L, D)
            # Cosine similarity; if not normalized, this is a dot product.
            sim = (v1 * v2).sum(dim=-1).mean(dim=0)  # (L,)
            return idx_a, idx_b, sim

        # Compute each position's pruning redundancy score.
        ...
        for g in groups:
            ...
            m = len(frs)
            if m == 1:
                # Single-frame group -> key frame by default; use audio scoring if enabled.
                key_indices = [0]
                non_key = []
            else:
                ...
                # Frame-level non-key score: average similarity to adjacent frames.
                frame_scores = []
                for j in range(m):
                    vals = []
            ...
            # Non-key frames: score with the aligned adjacent frame toward the nearest key frame.
            for j in non_key:
                # Choose the nearest key-frame direction: left or right.
                ...
                if left_dist <= right_dist and left_dist != 10**9:
                    # Use left-neighbor similarity aligned to the current frame.
                    ia, ib, sim = pairwise_sims(frs[j - 1], frs[j])
                    if ib is not None:
                        scores[ib] = torch.maximum(scores[ib], sim)
                elif right_dist != 10**9:
                    # Use right-neighbor similarity aligned to the current frame.
                    ia, ib, sim = pairwise_sims(frs[j], frs[j + 1])
                    if ia is not None:
                        scores[ia] = torch.maximum(scores[ia], sim)
                else:
                    # No key frame; this should not happen because there is at least one key frame.
                    continue

            # Key frames: if enabled, score with key-frame vs. following-audio similarity.
            if keyframe_audio_mode != "none" and g["audio"] is not None:
                a_s, a_e = g["audio"]
                if a_e >= a_s:
                    # Mean audio vector across batch and time.
                    audio_vec = emb[:, a_s:(a_e + 1), :].mean(dim=(0, 1), keepdim=False)  # (D,)
                    # Safely normalize here if normalization was not done earlier.
                    if not normalize:
                    ...
                    for j in key_indices:
                        ...
                        v = emb[:, idx, :].mean(dim=0)  # (L,D) -> per-position vector or mean?
                        # Per-position scoring: dot product/cosine between each position and audio_vec.
                        v_pos = emb[:, idx, :].mean(dim=0)  # (L,D), averaged across batch.
                        if not normalize:
                            # Convert to cosine similarity.
                            v_pos = v_pos / (v_pos.norm(dim=-1, keepdim=True) + 1e-6)
                        # Similarity scores, shape (L,).
                        sim_k = (v_pos * audio_vec.unsqueeze(0)).sum(dim=-1)

                        if keyframe_audio_mode == "prune_high":
                            # Higher similarity means more redundancy and is more likely to be pruned.
                            scores[idx] = torch.maximum(scores[idx], sim_k)
                        elif keyframe_audio_mode == "prune_low":
                            # Lower similarity means more redundancy; use negative similarity as score.
                            scores[idx] = torch.maximum(scores[idx], -sim_k)
                        # Ignore other values.

        # Sort only video positions.
        ...
        # Replace invalid (-inf) scores with a very small value to avoid selection.
        valid = torch.isfinite(vid_scores)
        if valid.any():
            # Use a very small placeholder so topk will not select it.
            ...
            # If no valid score exists, which is rare, do not prune.
            return torch.ones(N, device=device, dtype=torch.bool)

        # Select positions to prune.
